...
const MemberList = () => {
  ...
  useEffect(() => {
    ...
    fetch('https://geektrust.s3-ap-southeast-1.amazonaws.com/adminui-problem/members.json')
      ...
      .catch((error) => {
        console.error('Error fetching members from api call:', error);
      });
  }, []);
  ...
  const displayMemberRow = (member) => {
    ...
    return (
      <tr key={member.id} style={{ backgroundColor: isSelected ? '#f0f0f0' : 'inherit' }}>
        ...
        <td>{isEditing ? <input defaultValue={member.role} onBlur={(e) => handleSaveButon(member.id, { role: capitalizeFirstLetter(e.target.value) })} /> : capitalizeFirstLetter(member.role)}</td>
        <td>
          {isEditing ? (
  ...
  return (
    <div className='member-list-container'>
      ...
      <table className='member-table'>
        ...
        <tbody>
          {displayedMembers.map(displayMemberRow)}
        </tbody>
      </table>
